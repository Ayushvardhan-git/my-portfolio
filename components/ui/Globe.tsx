...
export function Globe({ globeConfig, data }: WorldProps) {
  ...
  useEffect(() => {
    if (globeRef.current && globeData) {
      ...
      const validFeatures = countries.features.filter((feature) => {
        // Ensure the feature has valid geometry
        return (
          feature.geometry &&
          ...
          feature.geometry.coordinates.every(
            (coord) =>
              Array.isArray(coord) &&
              coord.every((num) => typeof num === "number" && !isNaN(num))
          )
        );
      });
      ...
      // Ensure all coordinates in data are valid numbers
  ...
  const startAnimation = () => {
    ...
    const validArcsData = data.filter(
      (d) =>
        !isNaN(d.startLat) &&
        !isNaN(d.startLng) &&
        !isNaN(d.endLat) &&
